...
exports.exportWeeklyReportMarkdown = exportWeeklyReportMarkdown;
...
function exportWeeklyReportMarkdown(report) {
    const lines = [
...
function getGitCommitsForWeek(projectPath, weekStart, weekEnd) {
    try {
        const output = (0, child_process_1.execSync)(`git log --since="${weekStart}" --until="${weekEnd}T23:59:59" --format="%H|%aI|%s" --name-only`, { cwd: projectPath, encoding: 'utf-8', stdio: ['pipe', 'pipe', 'ignore'], timeout: 10000 });
        const commits = [];
...
function findCompletedTasks(tasks, commits, projectPath) {
    const completed = [];
...
function isTaskCompleted(task, changedFiles, projectPath) {
    const evidenceKeywords = task.completion_criteria.toLowerCase();
    for (const file of changedFiles) {
        const lower = file.toLowerCase();
        if (evidenceKeywords.includes('readme') && lower.includes('readme'))
            return true;
        if (evidenceKeywords.includes('plugin') && lower.includes('openclaw.plugin'))
            return true;
        if (evidenceKeywords.includes('测试') && (lower.includes('test') || lower.includes('spec')))
            return true;
        if (evidenceKeywords.includes('文档') && (lower.includes('.md') || lower.includes('doc')))
            return true;
    }
    return false;
}
function findTaskEvidence(task, changedFiles) {
    const evidence = [];
    const keywords = task.name.toLowerCase();
    for (const file of changedFiles) {
        const lower = file.toLowerCase();
        if ((keywords.includes('index') && lower.includes('index')) ||
...
function suggestNextWeekFocus(tasks, progress) {
    const suggestions = [];
...
function identifyBlockers(progress) {
    const blockers = [];
