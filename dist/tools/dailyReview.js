...
exports.dailyReview = dailyReview;
...
function dailyReview(params) {
    const { project_path, plan } = params;
    const review_date = new Date().toISOString().split('T')[0];
    // 1. 检查当前进度
    const cfg = (0, projectConfig_1.resolveConfig)(project_path, {});
    const progress = (0, progressScorer_1.checkProgress)(project_path, cfg.evidence_rules, cfg.exclude_patterns);
    // 2. 分析进度趋势
    let trend;
    try {
        const history = (0, progressHistory_1.loadProgressHistory)(project_path);
        if (history && history.snapshots && history.snapshots.length > 0) {
            trend = (0, progressHistory_1.analyzeProgressTrend)(project_path);
        }
    }
    catch (e) {
        // 忽略历史加载错误
...
function identifyTodayFocus(progress, plan) {
    const focus = [];
    // 基于优先级和进度确定今日重点
    ...
    if (plan && plan.micro_tasks) {
        ...
        const todayTasks = plan.micro_tasks.filter((t) => {
...
function identifyOverdueTasks(progress, plan) {
    const overdue = [];
    if (plan && plan.micro_tasks) {
        const today = new Date().toISOString().split('T')[0];
        const overdueTasks = plan.micro_tasks.filter((t) => {
...
function assessRiskLevel(progress, trend) {
...
function generateSummary(progress, trend, todayFocus, overdueTasks) {
    const lines = [];
...
function generateNextActions(progress, riskLevel) {
    const actions = [];
    if (riskLevel === 'high') {
