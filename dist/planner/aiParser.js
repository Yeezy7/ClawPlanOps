...
exports.aiParseTaskRequirements = aiParseTaskRequirements;
 ...
 */
async function aiParseTaskRequirements(options) {
    ...
    const data = json;
    return {
        task_name: data.task_name || '未命名任务',
        deadline: normalizeDeadline(data.deadline),
        deliverables: Array.isArray(data.deliverables) ? data.deliverables : [],
        constraints: Array.isArray(data.constraints) ? data.constraints : [],
        submission_rules: Array.isArray(data.submission_rules) ? data.submission_rules : [],
...
function normalizeDeadline(deadline) {
    ...
    const m = deadline.match(/(\d{4})[-\/年](\d{1,2})[-\/月](\d{1,2})[日]?[T\s]?(\d{1,2}):?(\d{2})?/);
    if (m) {
