 ...
 */
function reschedulePlan(progressReport, originalPlan, deadline) {
    const totalDays = originalPlan.total_days;
    const elapsedDays = daysSinceStart(originalPlan.start_date);
    const remainingDays = (0, taskParser_1.calcAvailableDays)(deadline);
    ...
    const delayDays = Math.max(0, Math.round(((expectedPercent - actualPercent) / 100) * totalDays));
    const isOnTrack = actualPercent >= expectedPercent - 10; // 10% tolerance
    if (isOnTrack) {
        return {
            delay_days: 0,
            is_on_track: true,
            priority_tasks: originalPlan.micro_tasks.filter((t) => t.priority === 'high'),
            removed_tasks: [],
            compressed_tasks: [],
            recommended_next_plan: originalPlan.phases.filter((p) => new Date(p.end_date) >= new Date()),
    ...
    const priorityTasks = allTasks.filter((t) => t.priority === 'high' &&
        !removedTasks.some((r) => r.id === t.id));
    // Generate compressed phases
    const keptTasks = [
        ...priorityTasks,
        ...compressedTasks.filter((t) => !removedTasks.some((r) => r.id === t.id)),
    ];
    const advice = [];
    if (delayDays > 0) {
    ...
    if (removedTasks.length > 0) {
        advice.push(`建议删除 ${removedTasks.length} 个低优先级任务: ${removedTasks
            .map((t) => t.name)
            .join(', ')}`);
    }
    if (compressedTasks.length > 0) {
        advice.push(`建议压缩 ${compressedTasks.length} 个中低优先级任务的时间预估`);
    }
    advice.push(`剩余 ${remainingDays} 天，优先完成高优先级任务共 ${priorityTasks.length} 个`);
    // Generate recommended next plan phases
    const recommendedPhases = rebuildPhases(keptTasks, originalPlan.phases, remainingDays);
    return {
...
function rebuildPhases(tasks, originalPhases, remainingDays) {
    const taskIds = new Set(tasks.map((t) => t.id));
    const relevantPhases = originalPhases.filter((p) => tasks.some((t) => t.phase_name === p.phase_name));
    // Adjust phase dates
    const adjusted = [];
...
//# sourceMappingURL=rescheduler.js.map