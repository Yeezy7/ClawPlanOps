...
function extractDeadline(text) {
    ...
    for (const pat of criticalPatterns) {
        ...
        if (m) {
            ...
            const dateM = afterText.match(/(\d{4})\s*年\s*(\d{1,2})\s*月\s*(\d{1,2})\s*日\s*(\d{1,2}:\d{2})?/);
            if (dateM) {
...
function extractDeliverables(text) {
    ...
    const cleaned = deliverables
        ...
        if (/^(以下|如下|包括|包含|含|需|需要|提交|各|请|参赛|所有|作品)/.test(d))
            return false;
...
function extractConstraints(text) {
    ...
    const sectionMatch = text.match(/(?:参赛要求|报名条件|注意事项|基本要求)[：:\s]*([\s\S]{30,500}?)(?:\n\n|\n[五六七八九]|$)/);
    ...
    for (const line of lines) {
        ...
        if (clean.length > 6 && clean.length < 100 && !clean.startsWith('--')) {
            ...
            if (/不超过|不多于|不少于|至少|需|必须|不得|应当|限报|须为|不得/.test(clean)) {
...
function extractSubmissionRules(text) {
    ...
    const sectionMatch = text.match(/(?:提交方式|提交地址|报送方式)[：:\s]*([\s\S]{30,400}?)(?:\n\n|\n[五六七八九]|$)/);
    const searchText = sectionMatch ? sectionMatch[1] : text;
