import type { EvidenceResult, ProgressReport, RiskLevel } from '../types';
import { mergeRules } from './evidenceRules';
import { scanEvidence } from './fileScanner';
import { checkGitEvidence } from './gitEvidence';
...
function identifyRisks(missing: EvidenceResult[]): string[] {
  const risks: string[] = [];

  const criticalMissing = missing.filter((r) => r.weight >= 10);
  if (criticalMissing.length > 0) {
    risks.push(
      `关键缺失项 (${criticalMissing.length}): ${criticalMissing
        .map((r) => r.description)
        .join('; ')}`
    );
  }
  ...
  const hasCode = missing.some((r) =>
    ['E-005', 'E-006'].includes(r.rule_id)
  );
  if (hasCode) {
...
function calculateRiskLevel(
  ...
  if (percent < 25 || missingHighWeight >= 5) return 'critical';
  if (percent < 50 || missingHighWeight >= 3) return 'high';
  if (percent < 75 || missing.length >= 4) return 'medium';
  return 'low';
}
