import type { MicroTask, DeliverableItem, Phase } from '../types';
...
function estimateMinutes(taskName: string, dailyHours: number): number {
  const name = taskName.toLowerCase();

  if (name.includes('测试') || name.includes('集成')) return 90;
  if (name.includes('创建') || name.includes('初始化')) return 45;
  if (name.includes('编写') || name.includes('实现') || name.includes('制作')) return 60;
  if (name.includes('排练') || name.includes('录制')) return 90;
  if (name.includes('剪辑') || name.includes('配音')) return 90;
  if (name.includes('填写') || name.includes('排版')) return 45;
  if (name.includes('检查') || name.includes('确认') || name.includes('校对')) return 30;
  if (name.includes('大纲') || name.includes('脚本') || name.includes('定义')) return 30;

  // default: one daily block
  return dailyHours * 30;
}

function buildCriteria(subTask: string, deliverable: DeliverableItem): string {
