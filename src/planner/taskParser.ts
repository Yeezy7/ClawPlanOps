...
function extractDeadline(text: string): string {
  ...
  let bestDate: DateMatch | null = null;
  ...
  if (bestDate) return bestDate.iso;
...
function findAllDates(text: string): DateMatch[] {
  ...
  for (const m of text.matchAll(reShort)) {
    ...
    if (/\d{4}\s*年\s*$/.test(before)) continue;
    results.push({
      full: m[0],
  ...
  for (const r of results) {
    ...
    if (timeMatch && timeMatch.index! < 15) {
      ...
      if (afterText.includes('下午') && hour < 12) hour += 12;
...
function extractDeliverables(text: string): string[] {
  ...
  for (const m of text.matchAll(sendPat)) {
    ...
    if (item.length > 2 && item.length < 50) found.push(item);
  }
  ...
  //   - README.md 项目说明文档
  const bulletSectionMatch = text.match(
    /(?:交付物|提交材料|需提交|需准备|需报送|报名材料|提交以下)[^\n]{0,30}[：:\s]*\n((?:[^\n]*[-•*]\s+\S+[^\n]*\n?)+)/
  );
    ...
    for (const line of lines) {
      ...
      if (bulletMatch) {
        const item = bulletMatch[1]
          ...
          .replace(/[【】\[\]《》「」『』]/g, '')
          .trim();
        if (item.length > 2 && item.length < 80) found.push(item);
      }
    }
  }
  ...
  return [...new Set(found)]
    ...
    .filter((d) => {
      if (d.length < 2 || d.length > 80) return false;
      if (/^(以下|如下|包括|包含|需|需要|提交|各|请|参赛|所有|作品|更多|欢迎)/.test(d)) return false;
