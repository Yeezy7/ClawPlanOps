 ...
 */
export function exportMarkdownPlan(plan: DeliverablePlan): string {
  const lines: string[] = [
  ...
  for (const phase of plan.phases) {
    ...
    const phaseTasks = plan.micro_tasks.filter(
      (t) => t.phase_name === phase.phase_name
    );
    if (phaseTasks.length > 0) {
      ...
      for (const t of phaseTasks) {
        ...
        lines.push(
          `| ${t.name} | ${t.estimated_minutes}min | ${prio} | ${t.completion_criteria} |`
        );
      }
      lines.push('');
    }
  ...
  for (const d of plan.deliverables) {
    ...
    lines.push(
      `| ${d.id} | ${d.name} | ${prio} | ${d.evidence_files.join(', ')} |`
    );
  }
  lines.push('');
 ...
 */
export function exportMarkdownProgress(report: ProgressReport): string {
  const lines: string[] = [
    `# 📊 进度证据检查报告`,
    '',
 ...
 */
export function exportMarkdownReschedule(result: RescheduleResult): string {
  const lines: string[] = [
    `# 🔄 动态重排建议`,
    '',
